...
if __name__ == '__main__':
    w, pred, target = 1, 5, 1 # power generated , prédiction t+5
    #w, pred, target = 1, 0, 1 # electricity load
    ...
    if True:
        ...
        # train_model(w, pred, name, data, idx, target, epoch=100, lr=1e-6, noise=None, sn=None, batch=16, memory=1.)
        # train_model(w, pred, trained, data, idx, t_pos[target],
                    # epoch=400, lr=1e-6, noise=1e-2, sn=sep_noise, batch=128, memory=.2 / 11)

        train_model(w, pred, trained, data, idx, t_pos[target],
                    epoch=500, lr=1e-6, noise=3e-7, sn=sep_noise, batch=120)
